...
(defcomponent :orthographic-camera [{:keys [left right top bottom near far zoom active]
                                     :or   {near   0.1
                                            far    2000.0
                                            active true
                                            zoom   1}}]
  (let [cam (three/OrthographicCamera. left right top bottom near far)]
    (set! (.-active cam) active)
    (set! (.-zoom cam) zoom)
    ;FIXME call .updateProjectionMatrix after position is set? in add-node and update-node1
